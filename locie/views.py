    ...
    def post(self, request, format=None):
        if not Store.objects.get(store_key=storeKeyGenerator(request.POST['servei_id'])):
            ...
            store = Store.objects.create(store_key=storeKeyGenerator(servei.servei_id),
                                         ...
                                         creator = servei.servei_id,owners = [servei.servei_id],official_support= request.POST['official_support'],
                                         store_category=request.POST['store_category'],father_categories=request.POST['father_categories'],contacts=request.POST['contacts'],
            ...
            servei.save()
            coords = Coordinates(unique_id=coord_id_generator(store.cityCode,store.store_key),relation='904',reference_id=store.store_key,cityCode=store.cityCode)
            store.coordinates = coords.uniqueId
            store.save()
...
class OtpCreator(APIView):
    ...
    def get(self, request, format=None):
        if request.GET['phone_number'] is not None and request.GET['otp_cred'] == '' and request.GET['data'] == '':
            ...
            log = OTPLog.objects.create(otp_cred = request.GET['phone_number'] + data,data=data,phone_number=request.GET['phone_number'])
            ...
            if dtime_diff(now,log.created)<= 6*60 :
                ...
                log.save()
                token=PhoneToken.objects.create(token=token_urlsafe(16),phone_number=log.phone_number)
...
class CityCodeCreator(APIView):
    ...
    def post(self, request, format=None):
        if not CityCode.objects.get(cityCode =request.POST['cityCode']):
            city =CityCode.objects.create(city=rquest.POST['city'],state=request.POST['state'],cityCode=request.POST['cityCode'],pin_codes=[request.POST['pin_code']])
            serial_code = CityCodeSerializer()
            if serial_code.is_valid():
                serial_code.save()
                return Response(serial_code.data,status=status.HTTP_201_CREATED)
        elif CityCode.objects.get(cityCode = CityCode().cityCoder(pin_code=request.POST['pin_code'])):
