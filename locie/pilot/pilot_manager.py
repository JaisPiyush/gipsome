...
from ..gadgets.rpmns import send_notification_to_partner
...
class PilotManager:
    ...
    def pilot_compass(self, first=True):
        ...
        if final_pilot:
            ...
            if self.order.delivery_type == 'SSU':
                self.order.locie_reversion = sum(
                    [value['net_price'] for value in self.order.final_servei_cluster.values()])
    ...
    def route_planner(self, cust_to_servei=False):
        ...
        if not cust_to_servei:
            ...
            customer_coord = Point(self.order.customer_stack['coordinates']['lat'],
                                   self.order.customer_stack['coordinates']['long'])
            pointers = []
            for serveiId in servei_list:
    ...
    def next_task(self):
        ...
        returning_data['primary'] = {
            ...
            "otp": order.otp,
            "net_charge": math.ceil(order.extra_charge['delivery_charge'] / 2) if order.delivery_type == 'UDS' else
            order.extra_charges['delivery_charge'],
